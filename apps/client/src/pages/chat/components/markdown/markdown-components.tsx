...
export const createMarkdownComponents = (
  ...
  return {
    ...
    pre: ({ children, ...props }: { children?: React.ReactNode }) => (
      <pre
        className="mb-2 p-2 rounded bg-black bg-opacity-10 overflow-x-auto"
        {...props}
      >
        {children}
      </pre>
    ...
    code: ({
      ...
      if (isInline) {
        return (
          <code
            className="px-1 py-0.5 rounded bg-black bg-opacity-10 text-xs font-mono"
      ...
      return (
        <code
          className={`block text-xs font-mono whitespace-pre ${className || ''}`}
          {...(props as React.HTMLAttributes<HTMLElement>)}
        >
          {children}
        </code>
    ...
    blockquote: ({ ...props }) => (
      <blockquote
        className="border-l-4 border-opacity-30 pl-3 py-1 my-2 italic opacity-80"
        {...props}
      />
    ),
    ...
    table: ({ ...props }) => (
      <div className="overflow-x-auto my-2">
        <table
          className="border-collapse border border-opacity-20 min-w-full"
          {...props}
        />
      </div>
    ),
    thead: ({ ...props }) => (
      <thead className="bg-black bg-opacity-10" {...props} />
    ),
    tbody: ({ ...props }) => <tbody {...props} />,
    tr: ({ ...props }) => (
      <tr className="border-b border-opacity-20" {...props} />
    ),
    th: ({ ...props }) => (
      <th
        className="border border-opacity-20 px-2 py-1 text-left font-semibold text-xs"
        {...props}
      />
    ),
