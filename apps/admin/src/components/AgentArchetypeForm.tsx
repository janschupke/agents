...
export default function AgentArchetypeForm({
  ...
  const createMutation = useMutation({
    mutationFn: (data: Parameters<typeof AgentArchetypeService.createArchetype>[0]) =>
  ...
  const validate = (): boolean => {
    ...
    if (formValues.temperature && (Number(formValues.temperature) < 0 || Number(formValues.temperature) > 2)) {
      newErrors.temperature = t('archetypes.form.errors.temperatureRange');
    }

    if (formValues.age && (Number(formValues.age) < 0 || Number(formValues.age) > 100)) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-background-secondary rounded-lg p-6 border border-border">
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          <h4 className="text-sm font-medium text-text-secondary">{t('archetypes.form.basicInfo')}</h4>

          <div>
            <label className="block text-sm font-medium text-text-primary mb-1">
              {t('archetypes.form.name')} <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                onChange={(e) => setFormValues({ ...formValues, agentType: e.target.value as AgentType | '' })}
                className="w-full px-3 py-2 border border-border rounded-md bg-background text-text-primary"
              >
                <option value="">{t('archetypes.form.selectAgentType')}</option>
                <option value={AgentType.GENERAL}>{AgentType.GENERAL}</option>
                <option value={AgentType.LANGUAGE_ASSISTANT}>{AgentType.LANGUAGE_ASSISTANT}</option>
              </select>
            </div>
        ...
        {/* Configuration */}
        <div className="space-y-4">
          <h4 className="text-sm font-medium text-text-secondary">{t('archetypes.form.configuration')}</h4>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) => setFormValues({ ...formValues, temperature: e.target.value })}
                className={`w-full px-3 py-2 border rounded-md bg-background text-text-primary ${
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setFormValues({ ...formValues, maxTokens: e.target.value })}
                className={`w-full px-3 py-2 border rounded-md bg-background text-text-primary ${
                  errors.maxTokens ? 'border-red-500' : 'border-border'
                }`}
              />
              {errors.maxTokens && <p className="text-red-500 text-xs mt-1">{errors.maxTokens}</p>}
            </div>
          </div>
        ...
        <div className="space-y-4">
          <h4 className="text-sm font-medium text-text-secondary">{t('archetypes.form.personality')}</h4>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                onChange={(e) => setFormValues({ ...formValues, responseLength: e.target.value as ResponseLength | '' })}
                className="w-full px-3 py-2 border border-border rounded-md bg-background text-text-primary"
              >
                <option value="">{t('archetypes.form.selectResponseLength')}</option>
                <option value={ResponseLength.SHORT}>{ResponseLength.SHORT}</option>
                <option value={ResponseLength.STANDARD}>{ResponseLength.STANDARD}</option>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              >
                ...
                <option value={Gender.PREFER_NOT_TO_SAY}>{Gender.PREFER_NOT_TO_SAY}</option>
              </select>
            </div>
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setFormValues({ ...formValues, personality: e.target.value as PersonalityType | '' })}
                className="w-full px-3 py-2 border border-border rounded-md bg-background text-text-primary"
              >
                <option value="">{t('archetypes.form.selectPersonality')}</option>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                onChange={(e) => setFormValues({ ...formValues, sentiment: e.target.value as Sentiment | '' })}
                className="w-full px-3 py-2 border border-border rounded-md bg-background text-text-primary"
              >
                ...
                <option value={Sentiment.DISINTERESTED}>{Sentiment.DISINTERESTED}</option>
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setFormValues({ ...formValues, availability: e.target.value as Availability | '' })}
                className="w-full px-3 py-2 border border-border rounded-md bg-background text-text-primary"
              >
                <option value="">{t('archetypes.form.selectAvailability')}</option>
                <option value={Availability.AVAILABLE}>{Availability.AVAILABLE}</option>
                <option value={Availability.STANDARD}>{Availability.STANDARD}</option>
